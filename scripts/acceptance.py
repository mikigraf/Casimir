...
def valid_report(returncode, report):
    checks=report.get('checks') or {}
    return returncode==0 and report.get('executionStatus')=='completed' and checks.get('outcome') in ['passed','failed'] and not (checks.get('outcome')=='failed' and report.get('overallOutcome')=='passed')

def permission_args(harness, unrestricted):
