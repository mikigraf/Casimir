...
use crate::util::{
    first_line, home_dir, ju64, now_iso, read_jsonl, read_jsonl_head, truncate, walk,
};
...
pub fn parse_records(records: &[Value], file: Option<&Path>) -> Session {
        ...
        match s(rec, "type").unwrap_or("") {
            ...
            "turn_context" => {
                ...
                if let Some(e) = p
                    ...
                    .and_then(|st| st.get("reasoning_effort"))
            ...
            "response_item" => match s(&p, "type").unwrap_or("") {
                    ...
                    match s(&p, "role") {
                        Some("user") => {
                            ...
                            if t.starts_with("<turn_aborted>") {
                                ...
                                let cmd = text
                                    .replace("<user_shell_command>", "")
                                    .replace("</user_shell_command>", "");
                                session
                                    .events
                                    .push(Event::system(cur, ts, "command", cmd.trim()));
                ...
                "local_shell_call" => {
                    ...
                    session
                        .events
                        .push(Event::tool_call(cur, ts, id, "shell", input));
                }
                "web_search_call" => {
                    let id = s(&p, "id")
                        .or_else(|| s(&p, "call_id"))
                ...
                "compacted" | "compaction" => {
                    session
                        .events
                        .push(Event::system(cur, ts, "compact", "context compacted"))
                }
                _ => {}
            },
            "compacted" => {
            ...
            "event_msg" => match s(&p, "type").unwrap_or("") {
                ...
                "task_complete" => {
                        ...
                        .and_then(Value::as_str)
                    {
                        ...
                        let dup = session
                            .events
                            .last()
                            .is_some_and(|l| l.kind == EventKind::Error && l.text_str() == msg);
                        if !dup {
                            session
                                .events
                                .push(Event::text(cur, ts, EventKind::Error, msg));
                        }
                    }
                }
                "error" | "stream_error" => {
                ...
                "turn_aborted" => {
                    session.events.push(Event::system(
                        ...
                        ts,
                        "interrupt",
                        format!("turn aborted ({})", s(&p, "reason").unwrap_or("unknown")),
                    ));
                }
    ...
    if session.id.is_empty() {
        if let Some(f) = file {
            session.id = id_from_filename(f).unwrap_or_else(|| {
                f.file_stem()
                    .and_then(|x| x.to_str())
                    .unwrap_or("")
                    .to_string()
...
pub fn list_sessions() -> Vec<SessionSummary> {
    ...
    for file in session_files() {
        ...
        let Ok(head) = read_jsonl_head(&file, 1024 * 1024) else {
            continue;
        };
        let sm = head
            .iter()
            .find(|r| s(r, "type") == Some("session_meta"))
            .and_then(|r| r.get("payload"));
        let first_user = head.iter().find(|r| {
            s(r, "type") == Some("response_item")
                && r.get("payload").is_some_and(|p| {
                    s(p, "type") == Some("message")
                        && s(p, "role") == Some("user")
                        && !is_injected(&text_of(p.get("content").unwrap_or(&Value::Null)))
                })
        });
        ...
        out.push(SessionSummary {
            ...
            id: sm
                .and_then(|m| s(m, "id").or_else(|| s(m, "session_id")))
                .map(String::from)
                .or_else(|| id_from_filename(&file))
                .unwrap_or_default(),
            cwd: sm.and_then(|m| s(m, "cwd")).map(String::from),
            git_branch: sm
                .and_then(|m| m.get("git"))
                .and_then(|g| s(g, "branch"))
                .map(String::from),
            started_at: sm
                .and_then(|m| s(m, "timestamp"))
                .map(String::from)
                .or_else(|| {
                    head.first()
                        .and_then(|r| s(r, "timestamp"))
                        .map(String::from)
                }),
...
pub fn exec_event_to_events(rec: &Value, turn: u32, state: &mut ExecState) -> Vec<Event> {
    ...
    match s(rec, "type").unwrap_or("") {
        ...
        "error" => evs.push(Event::text(
            turn,
            ts,
            EventKind::Error,
            s(rec, "message").unwrap_or("error"),
        )),
        "item.completed" => {
            ...
            match s(&it, "type").unwrap_or("") {
                ...
                "command_execution" => {
                    ...
                    let failed = it
                        ...
                        .and_then(Value::as_i64)
                        .is_some_and(|c| c != 0)
                        || s(&it, "status") == Some("failed");
                    evs.push(Event::tool_result(
                        turn,
                        ts,
                        id,
                        Some("shell".into()),
                        s(&it, "aggregated_output").unwrap_or(""),
                        failed,
                    ));
                }
                "file_change" => {
                ...
                "mcp_tool_call" => {
                    let name = format!(
                        "{}.{}",
                        s(&it, "server").unwrap_or("mcp"),
                        s(&it, "tool").unwrap_or("tool")
                    );
                    evs.push(Event::tool_call(
                        turn,
                        ts.clone(),
                    ...
                    let is_err = it.get("error").is_some_and(|e| !e.is_null())
                        || s(&it, "status") == Some("failed");
                    let out = it
                        .get("error")
                        .filter(|e| !e.is_null())
                ...
                "web_search" => evs.push(Event::tool_call(
                    turn,
                    ts,
                    id,
                    "web_search",
                    serde_json::json!({ "query": it.get("query").cloned().unwrap_or(Value::Null) }),
                )),
                "error" => evs.push(Event::text(
                    turn,
                    ts,
                    EventKind::Error,
                    s(&it, "message").unwrap_or("error"),
                )),
                _ => {}
            }
        }
...
fn sandbox_args(mode: Option<&str>) -> Vec<String> {
    match mode {
        ...
        Some("bypass") | Some("danger-full-access") => {
...
pub fn run_turn(opts: &RunOpts, on_event: &mut dyn FnMut(&Event)) -> Result<RunResult> {
    let bin = opts
        .bin
        .clone()
        .or_else(|| std::env::var("CASIMIR_CODEX_BIN").ok())
    ...
    let completed = state.thread_id.as_deref().is_some_and(|s| !s.is_empty())
        && res
            .raw
            .iter()
            .any(|r| s(r, "type") == Some("turn.completed"));
    res.is_error =
    ...
    if res.is_error && !res.events.iter().any(|e| e.kind == EventKind::Error) {
        let fallback = if status.success() {
            "codex ended without turn.completed or a thread ID"
        } else {
            "codex failed"
        };
        let ev = Event::text(
            ...
            now_iso(),
            EventKind::Error,
            crate::util::stderr_error_line(&res.stderr, fallback),
        );
        on_event(&ev);
        res.events.push(ev);
    }
            ...
            && rec.get("payload").is_some_and(|p| {
                s(p, "type") == Some("message") && s(p, "role") == Some("user") && {
                    ...
                    !text.trim().is_empty()
                        ...
                        && !text.trim_start().starts_with("<user_shell_command>")
                }
            });
        if is_user_turn {
            turn += 1;
