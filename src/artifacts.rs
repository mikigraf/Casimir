...
fn worktree_marker(path: &Path) -> Result<PathBuf> {
    ...
    if !output.status.success() {
        bail!("owned worktree Git metadata is missing");
    }
    Ok(PathBuf::from(String::from_utf8(output.stdout)?.trim()).join("casimir-owner"))
}
fn registry() -> PathBuf {
    crate::util::casimir_home().join("ownership")
}
pub fn register(run: &Path, workspace: &crate::rerun::WorkspacePlan) -> Result<()> {
    ...
    let worktree = workspace
        .root
        .as_ref()
        .filter(|_| workspace.mode == "worktree")
    ...
    let ownership = Ownership {
        ...
        run: run.clone(),
        worktree,
        repository: workspace.repo.clone(),
    };
    crate::util::atomic_write(&run.join(".casimir-owned"), id.as_bytes())?;
    if let Some(path) = &ownership.worktree {
        crate::util::atomic_write(&worktree_marker(path)?, id.as_bytes())?;
    }
...
fn verify(record: &Ownership) -> Result<()> {
    if record.schema_version != 1 || uuid::Uuid::parse_str(&record.id).is_err() {
        bail!("invalid ownership record");
    }
    if std::fs::canonicalize(&record.run)? != record.run || record.run.parent().is_none() {
        bail!("run ownership path changed");
    }
    if std::fs::read_to_string(record.run.join(".casimir-owned"))? != record.id {
        bail!("run ownership marker does not match");
    }
    if let Some(path) = &record.worktree {
        ...
        if !path.starts_with(&allowed) || path == &allowed {
            bail!("refusing worktree outside Casimir-owned storage");
        }
        ...
        if std::fs::canonicalize(path)? != *path {
            bail!("worktree ownership path changed");
        }
        if std::fs::read_to_string(worktree_marker(path)?)? != record.id {
            bail!("worktree ownership marker does not match");
        }
        let repo = record
            .repository
            .as_ref()
            .context("owned worktree has no repository")?;
        if std::fs::canonicalize(repo)? == *path {
            bail!("refusing to remove the original repository");
        }
    }
    Ok(())
}
...
pub fn cleanup_with_checkpoints(
    run: &Path,
    apply: bool,
    checkpoints: bool,
) -> Result<serde_json::Value> {
    if checkpoints
        && std::fs::symlink_metadata(run).is_err_and(|e| e.kind() == std::io::ErrorKind::NotFound)
    {
        return Ok(
            serde_json::json!({"schemaVersion":1,"preview":!apply,"run":run,"checkpoints":crate::checkpoint::cleanup_owner(run,apply)?,"note":"Run directory already removed; only registered checkpoint references are selected."}),
        );
    }
    ...
    for entry in std::fs::read_dir(registry()).context("no owned artifact registry")? {
        ...
        if record.run == canonical {
            found = Some((entry.path(), record));
            break;
        }
    }
    let (manifest, record) =
        found.context("run is not in the Casimir ownership registry; cleanup refused")?;
    verify(&record)?;
    let _lock = crate::util::RunLock::acquire_cleanup(&canonical)?;
    verify(&record)?;
    let checkpoint_preview = if checkpoints {
        Some(crate::checkpoint::cleanup_owner(&canonical, false)?)
    } else {
        None
    };
    ...
    if apply {
        if checkpoints {
            crate::checkpoint::cleanup_owner(&canonical, true)?;
        }
        if let Some(path) = record.worktree.as_ref().filter(|p| p.exists()) {
            ...
            let out = crate::process::capture(
                std::process::Command::new("git")
                    ...
                    .arg(path)
                    .current_dir(record.repository.as_ref().unwrap()),
                b"",
                std::time::Duration::from_secs(60),
                Some(&temp.path().join("cleanup")),
            )?;
            if !out.status.success() {
                bail!("Git refused removal of the owned worktree; run artifacts retained");
            }
        }
