...
pub fn pick_backend(requested: &str) -> String {
    ...
    if crate::doctor::subscription_ready(Harness::ClaudeCode) {
        return "claude-cli".into();
    }
    if crate::doctor::subscription_ready(Harness::Codex) {
...
fn request_api(
    ...
    let response = request
        .set("anthropic-beta", &betas.join(","))
        .send_json(body)
        .map_err(|e| match e {
            ureq::Error::Status(status, _) => {
                anyhow::anyhow!("Anthropic API HTTP {status} (response omitted for privacy)")
            }
            ureq::Error::Transport(_) => {
                anyhow::anyhow!("Anthropic API transport failed or timed out")
            }
        })?;
    ...
    if bytes.len() > 4 * 1024 * 1024 {
        bail!("API response exceeds 4 MiB");
    }
...
fn complete_cli(
    ...
    if !out.status.success() {
        bail!(
            ...
            out.status,
            crate::util::stderr_error_line(&out.stderr, "provider call failed")
        );
    }
    let last = stdout.trim().lines().last().unwrap_or("");
    let parsed: Value = serde_json::from_str(last).with_context(|| {
        format!(
            "claude -p returned no JSON ({}); raw output retained privately",
            out.status
        )
    })?;
    if parsed
...
fn complete_codex_cli(
    ...
    let working = tempfile::tempdir()?;
    let mut cmd = Command::new(&bin);
    cmd.current_dir(working.path()).args([
        "exec",
        "--json",
        "--ephemeral",
        "--ignore-user-config",
    ...
    if !out.status.success() {
        bail!(
            ...
            crate::util::stderr_error_line(
                &out.stderr,
                "check `codex login status` and your subscription"
            )
        );
    }
        ...
        .filter(|line| !line.is_empty())
    {
        let value: Value = serde_json::from_slice(line)
            .context("malformed Codex JSON stream; raw bytes retained privately")?;
        match value.get("type").and_then(Value::as_str) {
            ...
            Some("turn.failed") | Some("error") => {
                bail!("Codex judge/simulator call failed; inspect private process log")
            }
            _ => {}
        }
    }
    if !completed {
        bail!(
            "Codex judge/simulator call ended without turn.completed; inspect private process log"
        );
    }
    let text = final_text
...
fn complete_cmd(
    ...
    let mut cmd = Command::new(&bin);
    cmd.env("CASIMIR_LLM_SYSTEM", system)
    ...
    if !out.status.success() {
        bail!(
            "custom LLM command failed with {}: {}",
            out.status,
            crate::util::stderr_error_line(&out.stderr, "command failed")
        );
    }
    Ok(Completion {
...
pub fn effective_model(o: &LlmOpts) -> String {
    o.model.clone().unwrap_or_else(|| {
        if pick_backend(&o.backend) == "codex-cli" {
            "codex-cli-default".into()
        } else {
            DEFAULT_MODEL.into()
        }
    })
...
pub fn complete(system: &str, prompt: &str, o: &LlmOpts) -> Result<String> {
    ...
    let spool = o
        .recording_dir
        .clone()
        .unwrap_or_else(|| crate::util::casimir_home().join("llm"))
    ...
    let result = match backend.as_str() {
        ...
        "codex-cli" => {
            complete_codex_cli(system, prompt, o.model.as_deref(), o.timeout_secs, &spool)
        }
        "cmd" => complete_cmd(system, prompt, &model, o.timeout_secs, &spool),
        other => bail!("unknown llm backend {other}"),
    };
    crate::util::write_json(
        &spool.join("call.json"),
        &json!({"schemaVersion":1,"backend":backend,"requestedModel":model,"durationMs":start.elapsed().as_millis(),
...
mod tests {
    ...
    fn authentication_and_rate_limit_errors_omit_response_secrets() {
        for status in [401, 403, 429] {
            ...
            let error = request_api(
                Credential::ApiKey("test-credential".into()),
                &url,
                json!({}),
                2,
                directory.path(),
            ...
            assert!(request.contains("x-api-key: test-credential"));
        }
    }
    #[test]
    fn provider_error_body_is_private_not_a_diagnostic() {
        let body = json!({"type":"error","error":{"message":"credential sk-secret was rejected"}})
            .to_string();
        ...
        let directory = tempfile::tempdir().unwrap();
        let error = request_api(
            Credential::ApiKey("sk-secret".into()),
            &url,
            json!({}),
            2,
            directory.path(),
        ...
        assert!(
            std::fs::read_to_string(directory.path().join("response.json"))
                .unwrap()
                .contains("sk-secret")
        );
        server.join().unwrap();
    ...
    fn malformed_and_partial_api_responses_are_retained_and_fail() {
        for response in [
            ...
            let (url, server) = server(response, Duration::ZERO);
            let directory = tempfile::tempdir().unwrap();
            assert!(request_api(
            ...
            .is_err());
            assert!(!std::fs::read(directory.path().join("response.raw"))
                .unwrap()
                .is_empty());
            server.join().unwrap();
        }
    ...
    fn api_deadline_and_usage_are_reported() {
        let (url, slow) = server(Vec::new(), Duration::from_millis(1200));
        let directory = tempfile::tempdir().unwrap();
        let start = std::time::Instant::now();
        assert!(request_api(
            Credential::Bearer("test".into()),
