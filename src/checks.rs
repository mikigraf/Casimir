...
pub fn freeze(source: &Path, run_dir: &Path, workspace: &Path) -> Result<(Definition, String)> {
    ...
    if definition.schema_version != 1 || definition.checks.is_empty() {
        bail!("checks require schemaVersion 1 and a nonempty checks array");
    }
    if definition
        .checks
        .iter()
        .any(|c| c.executable.trim().is_empty() || c.timeout_secs == 0)
    {
        bail!("each check requires an executable and positive timeoutSecs");
    }
    let frozen = run_dir.join("checks.definition.json");
    let run = std::fs::canonicalize(run_dir)?;
    let workspace = std::fs::canonicalize(workspace)?;
    if run.starts_with(&workspace) {
        bail!("run output containing frozen checks must be outside the agent workspace");
    }
...
pub fn execute(definition: &Definition, hash: &str, cwd: &Path, run_dir: &Path) -> Result<Results> {
    ...
    if crate::checkpoint::hash(&std::fs::read(run_dir.join("checks.definition.json"))?) != hash {
        bail!("frozen check definition was modified");
    }
    ...
    for (index, check) in definition.checks.iter().enumerate() {
        ...
        .and_then(crate::process::Process::finish);
        let (outcome, exit_status, error) = match output {
            Ok(output) => (
                if output.status.code() == Some(check.expected_exit_status) {
                    "passed"
                } else {
                    "failed"
                },
        ...
        if outcome == "failed" || (outcome == "error" && results.outcome != "failed") {
            results.outcome = outcome.into();
        }
        results.results.push(ResultRecord {
            index,
            outcome: outcome.into(),
            exit_status,
            expected_exit_status: check.expected_exit_status,
            duration_ms: start.elapsed().as_millis(),
            stdout: spool.join("stdout.log").display().to_string(),
            stderr: spool.join("stderr.log").display().to_string(),
            error,
        });
