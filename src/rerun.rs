...
use crate::compare::{
    compare_sessions, judge_sessions_with, render_compare_markdown, sequence_similarity, JudgeOpts,
    Report,
};
use crate::llm::{effective_model, LlmOpts};
use crate::model::{
    action_sequence, renumber_turns, stats, user_turns, Event, EventKind, Execution, Harness,
    RerunOf, Session, Simulated, SimulatorInfo,
};
use crate::render::{format_event, RenderOpts};
use crate::simulate::simulate_user_turn;
use crate::util::{
    casimir_home, colors, first_line, fmt_num, now_iso, now_stamp, pad, slug, write_json,
};
use crate::workspace::{
    base_commit, capture_diff, capture_diff_against, create_worktree, is_git_repo,
    reconstruct_original_diff, repo_root, Diff,
};
...
fn resolve_brief(
    ...
    let needed =
        !o.freeze_inputs && (o.judge || o.user_mode == "simulate" || o.user_mode == "auto");
    if !needed {
        return Ok(None);
    }
    log(&format!(
        "{}drafting a per-session brief (rubric, session analysis, intents) with {}…{}",
        c.magenta,
        effective_model(&o.judge_llm),
        c.reset
    ));
    match draft_brief(original, diff_a, &o.judge_llm) {
        Ok(b) => {
            ...
            fs::write(
                ...
                crate::brief::render_brief_markdown(&b),
            )?;
            log(&format!(
                "  brief saved to {} (edit it and pass --brief to reuse a human-reviewed version)",
                existing.display()
            ));
            Ok(Some(b))
        }
        Err(err) => {
            log(&format!(
                "{}could not draft a brief: {err}; the judge falls back to the generic prompt{}",
                c.yellow, c.reset
            ));
            Ok(None)
        }
...
pub fn plan_workspace_at(
    original: &Session,
    workspace: &str,
    run_id: &str,
    from_turn: Option<u32>,
) -> Result<WorkspacePlan> {
    let plain = |dir: PathBuf, mode: &str| WorkspacePlan {
        dir,
        mode: mode.into(),
        root: None,
        commit: None,
    ...
    if !matches!(workspace, "auto" | "worktree" | "same") {
        let dir = fs::canonicalize(workspace)
            .with_context(|| format!("workspace directory does not exist: {workspace}"))?;
        if !is_git_repo(&dir) {
            bail!("runs require a Git repository for reliable checkpoints");
        }
        ...
        return Ok(WorkspacePlan {
            ...
            mode: "worktree".into(),
            root: Some(root),
            commit: Some(crate::workspace::head_commit(&repo)?),
            how: Some("explicit repository HEAD".into()),
            repo: Some(repo),
            note: None,
            restored: None,
        });
    ...
    if workspace == "same" || (workspace == "auto" && !git) {
        ...
        if !git {
            p.note =
                Some("original cwd is not a git repo; running in place (no diff capture)".into());
        }
        return Ok(p);
    }
    ...
    if let Some(n) = from_turn {
        ...
        restored =
            Some("recorded repository workspace and conversation; external state excluded".into());
    }
...
fn merge_with_harness_log(session: &mut Session, full: Session) {
    let ours: Vec<Event> = session
        .events
        .iter()
        .filter(|e| e.kind == EventKind::User && !e.sidechain)
        .cloned()
        .collect();
    let source_turns: std::collections::HashMap<u32, u32> = full
        .events
        .iter()
        .filter(|e| e.kind == EventKind::User && !e.sidechain)
        .zip(&ours)
        .map(|(logged, ours)| (logged.turn, ours.source_turn.unwrap_or(ours.turn)))
        .collect();
    ...
    for e in &session.events {
        if (e.kind == EventKind::Error && !known.iter().any(|k| k == e.text_str()))
            || (e.kind == EventKind::System
                && e.subtype
                    .as_deref()
                    .is_some_and(|s| s == "fork" || s.starts_with("simulator-")))
        {
            merged.push(e.clone());
        }
    }
...
fn original_diff_for(original: &Session, o: &RerunOpts) -> Option<Diff> {
    o.original_diff.clone().or_else(|| {
        if o.freeze_inputs {
            None
        } else {
            reconstruct_original_diff(original)
        }
    })
}
...
fn validate_run(original: &Session, o: &RerunOpts) -> Result<()> {
    ...
    if o.replicates == 0 || o.judge_repeats == 0 {
        bail!("replicates and judge repeats must be at least 1");
    }
    if !o.pass_threshold.is_finite() || !(0.0..=10.0).contains(&o.pass_threshold) {
        bail!("pass threshold must be a finite score from 0 to 10");
    }
    if o.turn_timeout_secs == 0 || o.judge_llm.timeout_secs == 0 || o.sim_llm.timeout_secs == 0 {
    ...
    if let Some(t) = o.from_turn {
        if t < 2 || t as usize > o.turns.unwrap_or(n).min(n) {
            bail!("fork turn must be between 2 and the last requested turn");
        }
        if o.harness.unwrap_or(original.harness()) != original.harness() {
            bail!("fork-at-turn requires the same harness as the original");
        }
        if matches!(original.harness(), Harness::Copilot | Harness::Gemini) {
            bail!("fork-at-turn is not supported for {}", original.harness());
        }
    }
    Ok(())
}
...
fn rerun_inner(
    ...
    adapters::validate_permissions(&RunOpts {
    ...
    let model = recovery
        .as_ref()
        .and_then(|r| r.session.model.clone())
        .or_else(|| o.model.clone())
        .or_else(|| {
            if same_harness {
    ...
    let run_id = o
        .run_id
        .clone()
        .unwrap_or_else(|| make_run_id(original, harness, model.as_deref()));
    let turns = user_turns(original);
    if turns.is_empty() {
        bail!("original session has no user turns to replay");
    }
    let from_turn = recovery
        .as_ref()
        .map(|r| r.next_turn)
        .unwrap_or_else(|| o.from_turn.unwrap_or(1).max(1));
    if from_turn > turns.len() as u32 && recovery.is_none() {
        bail!(
            "cannot fork at turn {from_turn}: the session has {} user turns",
            turns.len()
        );
    }
    ...
    if let Some(repo) = &o.workspace_repo {
        workspace_original.cwd = Some(repo.display().to_string());
    }
    let restore_id = recovery
        .as_ref()
        .and_then(|r| r.checkpoint.clone())
        .or_else(|| {
            o.from_turn
                .and_then(|t| original.checkpoints.get(&t).cloned())
        });
    if o.from_turn.is_some() && restore_id.is_none() {
        bail!("fork requires a compatible checkpoint; timestamps and inferred commits are insufficient");
    }
    let checkpoint = restore_id
        .as_deref()
        .map(crate::checkpoint::load)
        .transpose()?;
    if let Some(cp) = &checkpoint {
        if from_turn as usize <= o.turns.unwrap_or(turns.len()).min(turns.len()) {
            crate::checkpoint::require_compatible(cp)?;
        }
    }
    let ws = if let Some(cp) = &checkpoint {
        let root = casimir_home().join("worktrees").join(&run_id);
        WorkspacePlan {
            dir: root.join(&cp.subdir),
            mode: "worktree".into(),
            root: Some(root),
            commit: Some(cp.base.clone()),
            how: Some("verified checkpoint".into()),
            repo: Some(crate::checkpoint::repository_path(cp)),
            note: Some(cp.coverage.clone()),
            restored: Some("checkpoint".into()),
        }
    } else {
        plan_workspace_at(&workspace_original, &o.workspace, &run_id, None)?
    };
    let max_turns = o.turns.map(|n| n.min(turns.len())).unwrap_or(turns.len());
    let run_dir = o
        .out_dir
        .clone()
        .unwrap_or_else(|| casimir_home().join("runs").join(&run_id));
    ...
    if run_dir.join("meta.json").exists() {
        bail!(
            "run directory already contains a run: {}; choose a fresh output directory",
            run_dir.display()
        );
    }
    ...
    log(&format!(
        ...
        model
            .as_ref()
            .map(|m| format!(" ({m})"))
            .unwrap_or_else(|| " (harness default model)".into())
    ));
    log(&format!(
        "  turns: {max_turns}/{}  user mode: {}{}",
        turns.len(),
        o.user_mode,
        if simulate {
            format!(
                " (simulator: {sim_model} via {})",
                crate::llm::pick_backend(&o.sim_llm.backend)
            )
        } else {
            String::new()
        }
    ...
    if from_turn > 1 {
        log(&format!(
            "  fork: turns 1..{} preserved from the original, turn {from_turn} {}",
            from_turn - 1,
            if o.intervention.is_some() {
                "replaced by the intervention message"
            } else {
                "resampled verbatim"
            }
        ));
    ...
    if o.dry_run {
        for t in turns.iter().take(max_turns).filter(|t| t.turn >= from_turn) {
            let text = if t.turn == from_turn && from_turn > 1 {
                o.intervention.as_deref().unwrap_or(&t.text)
            } else {
                &t.text
            };
            log(&format!(
                "  turn {}: {}{}",
                t.turn,
                first_line(text).chars().take(100).collect::<String>(),
                if t.turn == from_turn && o.intervention.is_some() {
                    "  (intervention)"
                } else {
                    ""
                }
    ...
    if run_dir.join("meta.json").exists() {
        bail!("run directory already contains a run");
    }
    for entry in fs::read_dir(&run_dir)? {
        let name = entry?.file_name();
        if name != ".lock" && !(recovery.is_some() && name == "recovery.json") {
            bail!("output directory must be empty to establish artifact ownership");
        }
    }
    if let Some(id) = &restore_id {
        ...
        create_worktree(
            ws.repo.as_ref().unwrap(),
            ws.commit.as_ref().unwrap(),
            ws.root.as_ref().unwrap(),
        )?;
        log(&format!(
            "  created worktree {}",
            ws.root.as_ref().unwrap().display()
        ));
    }
    ...
    let diff_base = if is_git_repo(&ws.dir) {
        Some(base_commit(&workspace_original, &ws.dir)?.0)
    } else {
        None
    };
    ...
    let frozen_checks = o
        .checks
        .as_ref()
        .map(|p| crate::checks::freeze(p, &run_dir, &ws.dir))
        .transpose()?;

    let mut session = recovery
        .as_ref()
        .map(|r| r.session.clone())
        .unwrap_or_else(|| Session::new(harness));
    ...
    if recovery.is_none() {
        session.execution = Some(Execution {
            ...
            preserved_turns: (from_turn - 1) as usize,
            ..Default::default()
        });
    }
    session.rerun_of = Some(RerunOf {
    ...
    if simulate {
        session.simulator = Some(SimulatorInfo {
            model: sim_model.clone(),
            backend: crate::llm::pick_backend(&o.sim_llm.backend),
        });
    }
    let mut meta = json!({
    ...
    let mut sim_state: crate::simulate::SimState = recovery
        .as_ref()
        .map(|r| r.simulator.clone())
        .unwrap_or_default();
    let configuration_for = |model: Option<&str>| -> Result<String> {
        Ok(crate::checkpoint::hash(&serde_json::to_vec(
            &json!({"harness":harness,"model":model,"permissionMode":permission_mode,"sandbox":sandbox,"extraArgs":o.extra_args,"checksHash":frozen_checks.as_ref().map(|(_, hash)| hash)}),
        )?))
    };
    ...
    if recovery.is_some()
        && checkpoint
            .as_ref()
            .is_some_and(|cp| cp.configuration_hash != configuration_hash)
    {
        bail!("recovery configuration differs from its checkpoint");
    }
    ...
    if from_turn > 1 && from_turn as usize <= max_turns {
        ...
        let cp = checkpoint
            .as_ref()
            .context("native continuation requires a verified checkpoint")?;
        ...
        session.events.push(Event::system(
            ...
            now_iso(),
            "fork",
            format!("continued from verified checkpoint before turn {from_turn}"),
        ));
    ...
    if frozen_checks.is_some() {
        saved_options.checks = Some(run_dir.join("checks.definition.json"));
    }
    if brief.is_some() {
        saved_options.brief = Some(run_dir.join("brief.json"));
    }
    let mut journal = crate::recovery::Recovery {
        schema_version: 1,
        state: "running".into(),
        options: saved_options,
        original: original.clone(),
        session: session.clone(),
        simulator: sim_state.clone(),
        next_turn: from_turn,
        checkpoint: restore_id.clone(),
        active: false,
    ...
    for (i, t) in turns.iter().take(max_turns).enumerate() {
        ...
        if t.turn == from_turn && recovery.as_ref().is_some_and(|r| r.active) {
            ...
            if let Some(m) = &o.intervention {
                ...
                simulated = Some(Simulated {
                    verbatim: false,
                    reason: "intervention: user message replaced at the fork".into(),
                    grounded_in: vec![from_turn],
                    action: Some("intervention".into()),
                });
            }
        } else if i > 0 && simulate {
            log(&format!(
                "{}simulating user for turn {}…{}",
                c.magenta, t.turn, c.reset
            ));
            let sim =
                match simulate_user_turn(original, &session, t.turn, &o.sim_llm, &mut sim_state) {
                    Ok(sim) => sim,
                    Err(err) => {
            ...
            match sim.message {
                None if sim.no_op => {
                    log(&format!(
                        "{}simulator: no-op at turn {} ({}){}",
                        c.magenta, t.turn, sim.reason, c.reset
                    ));
                    session.events.push(Event::system(
                        t.turn,
                        now_iso(),
                    ...
                    journal.session = session.clone();
                    journal.simulator = sim_state.clone();
                    journal.next_turn = t.turn + 1;
                    journal.save(&run_dir)?;
                    continue;
                }
                None => {
                    let reason = sim
                        .stop_reason
                        .clone()
                        .unwrap_or_else(|| "goals_met".into());
                    log(&format!(
                        "{}simulator stopped the session ({reason}): {}{}",
                        c.magenta, sim.reason, c.reset
                    ));
                    session.events.push(Event::system(
                        t.turn,
                        now_iso(),
                        &format!("simulator-stop:{reason}"),
                        sim.reason,
                    ));
                ...
                Some(m) => {
                    if !sim.verbatim {
                        log(&format!(
                            "{}adapted message [{}] (grounded in turns {:?}): {}{}",
                            c.magenta,
                            sim.kind.as_deref().unwrap_or("answer"),
                            sim.grounded_in,
                            first_line(&m).chars().take(120).collect::<String>(),
                            c.reset
                        ));
                    }
                    if sim.retries > 0 {
                        log(&format!(
                            "{}simulator needed {} retr{} to ground its message{}",
                            c.yellow,
                            sim.retries,
                            if sim.retries == 1 { "y" } else { "ies" },
                            c.reset
                        ));
                    }
                    simulated = Some(Simulated {
                        verbatim: sim.verbatim,
                        reason: sim.reason,
                        grounded_in: sim.grounded_in,
                        action: sim.kind,
                    });
                    message = m;
                }
        ...
        let native = harness_session_id
            .as_ref()
            .and_then(|id| adapters::find_log_by_id(harness, id));
        if is_git_repo(&ws.dir) {
            let cp = crate::checkpoint::capture(crate::checkpoint::Capture {
                cwd: &ws.dir,
                run_dir: &run_dir,
                ...
                turn: t.turn,
                expected_conversation_turns: user_turns(&session).len() as u32,
                prompt: &message,
                configuration_hash: &configuration_hash,
                limit: o.checkpoint_limit,
            })?;
        ...
        journal.session = session.clone();
        journal.simulator = sim_state.clone();
        journal.next_turn = t.turn;
        journal.active = true;
        journal.save(&run_dir)?;
        meta["activeTurn"] = json!(t.turn);
        write_json(&run_dir.join("meta.json"), &meta)?;
        ...
        let opts = RunOpts {
            ...
            model: model.clone(),
            session_id: if harness_session_id.is_none() {
        ...
        let mut res = match result {
            ...
            Err(err) => {
                live.push(Event::text(
                    ...
                    now_iso(),
                    EventKind::Error,
                    format!("harness execution failed: {err}"),
                ));
                adapters::RunResult {
                    is_error: true,
                    ..Default::default()
        ...
        if res.is_error {
            ...
            if !live.iter().any(|e| e.kind == EventKind::Error) {
                live.push(Event::text(
                    ...
                    now_iso(),
                    EventKind::Error,
                    "harness reported an unsuccessful turn",
                ));
            }
        } else {
            session.execution.as_mut().unwrap().completed_turns += 1;
        }
        if serde_json::to_vec(&session.events)?
            .len()
            .saturating_add(serde_json::to_vec(&live)?.len())
            > 256 * 1024 * 1024
        {
            bail!("run exceeds 256 MiB normalization limit; raw streams and recovery checkpoint retained");
        }
        ...
        session.cost_usd = if saw_cost && !missing_cost {
            Some(cost)
        } else {
            None
        };
        if !res.is_error {
            ...
            if is_git_repo(&ws.dir) {
                let native = harness_session_id
                    ...
                    .and_then(|id| adapters::find_log_by_id(harness, id));
                journal.checkpoint =
                    Some(crate::checkpoint::capture(crate::checkpoint::Capture {
                        cwd: &ws.dir,
                        run_dir: &run_dir,
                        native: native.as_deref(),
                        harness,
                        ...
                        turn: t.turn + 1,
                        expected_conversation_turns: user_turns(&session).len() as u32,
                        prompt: turns.get(i + 1).map(|t| t.text.as_str()).unwrap_or(""),
                        configuration_hash: &configuration_hash,
                        limit: o.checkpoint_limit,
                    })?);
                journal.save(&run_dir)?;
    ...
    for e in &mut session.events {
        e.source_turn.get_or_insert(e.turn);
    }
    if let Some(hid) = &harness_session_id {
        if let Some(file) = adapters::find_log_by_id(harness, hid) {
            match adapters::parse_file(harness, &file) {
                ...
                Ok(full) if user_turns(&full).len() >= user_turns(&session).len() => {
                    merge_with_harness_log(&mut session, full)
                }
                Ok(_) => {}
                Err(err) => {
    ...
    session.checkpoints = session
        ...
        .filter_map(|e| {
            session
                .checkpoints
                .get(&e.source_turn.unwrap_or(e.turn))
                .cloned()
                .map(|id| (e.turn, id))
        })
        .collect();
    ...
    let mut diff = diff_base
        .as_deref()
        .map(|base| capture_diff_against(&ws.dir, base))
        .unwrap_or_else(|| capture_diff(&ws.dir));
    diff.source = Some(format!(
        "captured from {} against {}",
        ws.dir.display(),
        diff_base.as_deref().unwrap_or("unavailable git base")
    ));
    fs::write(run_dir.join("diff.patch"), &diff.patch)?;
    write_json(
        &run_dir.join("diff.json"),
        &json!({ "schemaVersion": 1, "files": diff.files, "stat": diff.stat, "source": diff.source }),
    )?;
    if let Some(d) = &diff_a {
        fs::write(run_dir.join("original.patch"), &d.patch)?;
    }

    let check_results = frozen_checks
        .as_ref()
        .map(|(definition, hash)| crate::checks::execute(definition, hash, &ws.dir, &run_dir))
        .transpose()?;
    let mut judge = None;
    ...
    if o.judge {
        let jo = JudgeOpts {
            llm: o.judge_llm.clone(),
            repeats: o.judge_repeats.max(1),
            brief: brief.clone(),
            model_a: original.model.clone(),
            model_b: session.model.clone(),
        };
        judge_configuration_hash = Some(crate::compare::judge_configuration_hash(
            &jo,
            o.pass_threshold,
        ));
        log(&format!(
            "{}asking judge ({}) in both candidate orders{}…{}",
            c.magenta,
            effective_model(&o.judge_llm),
            if jo.repeats > 1 {
                format!(", {} repeats each", jo.repeats)
            } else {
                String::new()
            },
            c.reset
        ...
        match judge_sessions_with(original, &session, diff_a.as_ref(), Some(&diff), &jo) {
            ...
            Err(err) => {
                judge_error = Some(crate::privacy::redact(&err.to_string()));
                log(&format!("{}judge failed: {err}{}", c.red, c.reset));
            }
        }
    }
...
pub fn write_record(
    ...
    for (index, e) in session.events.iter().enumerate() {
        ...
        let inherited = session
            .execution
            .as_ref()
            .is_some_and(|x| e.source_turn.unwrap_or(e.turn) as usize <= x.preserved_turns);
        match e.kind {
            EventKind::Assistant => {
                model_n += 1;
...
fn entry_from(
    ...
    let start = session
        .execution
        .as_ref()
        .map(|e| e.preserved_turns as u32 + 1)
        .unwrap_or(1);
    let scoped = |s: &Session, align_source: bool| Session {
        events: s
            .events
            .iter()
            .cloned()
            ...
            .filter(|e| e.turn >= start && e.turn <= requested as u32)
            .collect(),
        ..s.clone()
    };
    ...
    let completed = session
        .execution
        .as_ref()
        .map(|e| e.preserved_turns + e.completed_turns + e.skipped_turns)
        .unwrap_or(0);
    let judge_score = report.judge.as_ref().map(|j| j.score_b);
    let end_state = report
        .end_state
        .as_ref()
        .filter(|e| e.informative)
        .map(|e| e.score);
    let stopped = session
        .execution
        .as_ref()
        .and_then(|e| e.stop_reason.as_deref());
    let all_turns_done =
        (completed >= requested && stopped.is_none()) || (stopped == Some("goals_met") && judged);
    let pass = continuation_errors == 0
        && session
            .execution
            .as_ref()
            .is_some_and(|e| e.failed_turns == 0)
        && all_turns_done
        && report.overall_outcome == "passed";
    // Compare only the executed suffix for forks; a long preserved prefix must not dilute distance.
    let distance =
        1.0 - sequence_similarity(&action_sequence(&reference), &action_sequence(&candidate));
    ReplicateEntry {
        ...
        control,
        control_model_verified: !control
            || original
                .model
                .as_deref()
...
pub fn summarize(
    ...
    let clean_group = |g: &GroupSummary| {
        entries
            .iter()
            .filter(|e| {
                e.control == g.control
                    && e.harness == g.harness
                    && e.model == g.model
                    && e.simulator_model == g.simulator_model
            })
            .all(|e| e.errors == 0 && e.completed_turns >= e.requested_turns)
    };
    let controls: Vec<(Option<String>, f64)> = groups
        .iter()
        .filter(|g| {
            g.control
                && clean_group(g)
                && entries
                    .iter()
                    .filter(|e| {
                        e.control && e.model == g.model && e.simulator_model == g.simulator_model
                    })
                    .all(|e| e.control_model_verified)
        })
        .map(|g| (g.simulator_model.clone(), g.max_distance))
        .collect();
    let mut notes: Vec<String> = Vec::new();
    if entries
        .iter()
        .any(|e| e.control && !e.control_model_verified)
    {
    ...
    for g in groups.iter_mut().filter(|g| !g.control) {
        if !clean_group(g) {
            notes.push(
                "control-spread comparison withheld for an incomplete or failed target group"
                    .into(),
            );
            continue;
        }
    ...
    for (h, m, control) in targets {
        let gs: Vec<&GroupSummary> = groups
            .iter()
            .filter(|g| {
                g.harness == h
                    && g.model == m
                    && g.control == control
                    && g.simulator_model.is_some()
            })
            .collect();
        if gs.len() >= 2 {
    ...
    if !judged {
        notes.push(
            "pass@1 measures clean execution only; task correctness has not been evaluated".into(),
        );
    }
...
pub fn render_matrix_text(m: &MatrixSummary) -> String {
    ...
    let mut out = vec![format!(
        "{}replicates: {} per group; pass = no errors, all turns, judge ≥ {:.1}{}{}",
        c.bold,
        m.replicates,
        m.pass_threshold,
        if m.judged {
            ...
            " (no evaluation evidence: task outcome is inconclusive)"
        },
        c.reset
    )];
    out.push(format!(
    ...
    for e in &m.entries {
        out.push(format!(
            "{}{}{}{}{}{}{}{}{}{}",
            pad(
                &format!("{}{}", e.label, if e.control { " (control)" } else { "" }),
                22
            ),
            ...
            pad(
                &e.judge_score
                    .map(|s| format!("{s:.1}{}", if e.order_sensitive { "*" } else { "" }))
                    .unwrap_or_else(|| "-".into()),
                7
            ),
            pad(
                &format!("{}/{}", f(e.end_state_score), f(e.end_state_recall)),
                11
            ),
            pad(
                &format!(
                    "{:.2}@{}",
                    e.tool_sequence_distance,
                    e.first_divergent_turn
                        .map(|t| t.to_string())
                        .unwrap_or_else(|| "-".into())
                ),
                10
            ...
            format_args!(
                ...
                if e.lucky { " (lucky)" } else { "" }
            )
        ));
    }
    if m.entries.iter().any(|e| e.order_sensitive) {
        out.push(format!(
            "{}* judge verdict flipped when candidate order was swapped{}",
            c.dim, c.reset
        ));
    }
    ...
    for g in &m.groups {
        let name = format!(
            "{}{}{}",
            g.harness.map(|h| h.to_string()).unwrap_or_default(),
            g.model
                .as_ref()
                .map(|m| format!("/{m}"))
                .unwrap_or_default(),
        ...
        out.push(format!(
            ...
            g.pass_at_1_ci.0,
            g.pass_at_1_ci.1,
            g.order_sensitive_rate
                .map(|r| format!("  judge order-sensitive rate {:.2}", r))
                .unwrap_or_default(),
            g.mean_judge_position_bias
                .map(|b| format!("  position bias {b:.2}"))
                .unwrap_or_default(),
            g.mean_judge_test_retest
                .map(|t| format!("  test-retest {t:.2}"))
                .unwrap_or_default(),
            g.mean_intent_coverage
                .map(|i| format!("  intent coverage {i:.2}"))
                .unwrap_or_default(),
        ));
    ...
    if !m.groups.iter().any(|g| g.control) && m.groups.len() > 1 {
        out.push(format!(
            "{}no control group: add --control to measure observed same-model variation{}",
            c.dim, c.reset
        ));
    }
    for n in &m.notes {
...
pub fn render_matrix_markdown(m: &MatrixSummary) -> String {
    let f = |v: Option<f64>| v.map(|x| format!("{x:.2}")).unwrap_or_else(|| "-".into());
    let mut md = vec![
        "# Replicated rerun".into(),
        String::new(),
        format!("- replicates per simulator model: {}", m.replicates),
        format!(
            "- pass criterion: no errors, all turns completed{}",
            if m.judged {
                format!(", judge score ≥ {:.1}", m.pass_threshold)
            } else {
                String::new()
            }
        ),
    ...
    for e in &m.entries {
        md.push(format!(
            ...
            e.simulator_model.as_deref().unwrap_or("-"),
            e.completed_turns,
            e.requested_turns,
            e.errors,
            e.tool_calls,
    ...
    if !m.simulator_spread.is_empty() {
        md.extend([
            String::new(),
            "| target | simulators | pass@1 spread | judge spread |".into(),
            "|---|---|---|---|".into(),
        ]);
        for sp in &m.simulator_spread {
            md.push(format!(
                "| {}{} | {} | {:.2}..{:.2} | {} |",
                sp.harness.map(|h| h.to_string()).unwrap_or_default(),
                sp.model
                    .as_ref()
                    .map(|m| format!("/{m}"))
                    .unwrap_or_default(),
                ...
                sp.pass_at_1_min,
                sp.pass_at_1_max,
                match (sp.judge_min, sp.judge_max) {
...
pub fn rerun_matrix(
    ...
    log: &mut dyn FnMut(&str),
    out: &mut dyn FnMut(&str),
) -> Result<(Option<RerunOutcome>, Option<MatrixSummary>)> {
    validate_run(original, o)?;
    if o.control
        && original
            .model
    ...
    let simulate = o.user_mode == "simulate" || o.user_mode == "auto";
    let sim_models: Vec<Option<String>> = if simulate && !o.sim_models.is_empty() {
        o.sim_models.iter().cloned().map(Some).collect()
    } else {
        vec![None]
    };
    ...
    let harness = o.harness.unwrap_or(original.harness());
    let model = o.model.clone().or_else(|| {
    ...
    let parent_dir = o
        .out_dir
        .clone()
        .unwrap_or_else(|| casimir_home().join("runs").join(&parent_id));
    ...
    let _experiment_lock = if o.dry_run {
        None
    } else {
        Some(crate::util::RunLock::acquire(&parent_dir)?)
    };
    let frozen = prepare_experiment(original, o, &parent_dir, log)?;
    let requested = o
        .turns
        .map(|n| n.min(user_turns(original).len()))
        .unwrap_or(user_turns(original).len());
    ...
    for (control, h, m) in &targets {
        for sm in &sim_models {
            for r in 1..=replicates {
                ...
                let sim_model = if simulate {
                    Some(effective_model(&single.sim_llm))
                } else {
                    None
                };
                entries.push(entry_from(
                    &label, original, *control, sim_model, requested, &outcome, o,
                ));
            }
        }
    }
    ...
    fs::write(
        parent_dir.join("report.md"),
        render_matrix_markdown(&summary),
    )?;
...
fn prepare_experiment(
    ...
    if o.workspace == "same" {
        bail!("replicates require independent worktrees; use --workspace auto or a git repository directory");
    }
    if !matches!(o.workspace.as_str(), "auto" | "worktree") {
        let repo = fs::canonicalize(&o.workspace)?;
        if !is_git_repo(&repo) {
            bail!("replicates require a git repository to create independent worktrees");
        }
        frozen.workspace_repo = Some(repo);
    } else if frozen.workspace_repo.is_none()
        && !original
            .cwd
            .as_deref()
            .is_some_and(|p| is_git_repo(Path::new(p)))
    {
        bail!("replicates require a git repository; pass --workspace <repository>");
    }
    frozen.workspace = "worktree".into();
    if !o.dry_run {
        if dir.join("replicates.json").exists() || dir.join("attribution.json").exists() {
            bail!("experiment directory already contains results: {}; choose a fresh output directory", dir.display());
        }
        fs::create_dir_all(dir)?;
        if let Some(source) = &o.checks {
            let workspace = frozen
                .workspace_repo
                .as_deref()
                .or_else(|| original.cwd.as_deref().map(Path::new))
                .context("experiment repository missing")?;
        ...
        if let Some(brief) =
            resolve_brief(original, &frozen, frozen.original_diff.as_ref(), dir, log)?
        {
...
pub fn attribute(
    ...
    validate_run(original, o)?;
    if !o.judge {
        bail!("attribution requires --judge: clean execution alone cannot establish that a failed task was rescued");
    }
    if o.harness.unwrap_or(original.harness()) != original.harness()
        || o.model
            .as_ref()
            .is_some_and(|m| Some(m) != original.model.as_ref())
    {
        bail!("attribution resamples the original harness and model; use fork for model-change interventions");
    }
    if original.model.is_none() {
        bail!("attribution requires the original model to be recorded");
    }
    ...
    if evaluation["outcome"] != "failed" || evaluation["execution"] != "completed" {
        bail!("attribution withheld: original must demonstrate task failure without infrastructure failure");
    }
    if evaluation["judgeModel"].as_str() != Some(effective_model(&o.judge_llm).as_str())
        || evaluation["passThreshold"].as_f64() != Some(o.pass_threshold)
    {
        bail!("attribution withheld: evaluation configuration differs from the original");
    }
    let rubric_hash = o
        .brief
        .as_ref()
        .map(|path| Brief::load(path).map(|b| crate::checkpoint::hash(b.rubric_text().as_bytes())))
        .transpose()?;
    ...
    let judge_options = JudgeOpts {
        llm: o.judge_llm.clone(),
        repeats: o.judge_repeats.max(1),
        brief: o.brief.as_ref().map(|p| Brief::load(p)).transpose()?,
        model_a: original.model.clone(),
        model_b: original.model.clone(),
    };
    if evaluation["judgeConfigurationHash"].as_str()
        != Some(crate::compare::judge_configuration_hash(&judge_options, o.pass_threshold).as_str())
    {
    ...
    let checks_hash = o
        ...
        .map(fs::read)
        .transpose()?
        .map(|bytes| crate::checkpoint::hash(&bytes));
    let configuration_hash = crate::checkpoint::hash(&serde_json::to_vec(
        &json!({"harness":original.harness(),"model":original.model,"permissionMode":o.permission_mode.as_deref().unwrap_or("preserve"),"sandbox":o.sandbox.as_deref().unwrap_or("preserve"),"extraArgs":o.extra_args,"checksHash":checks_hash}),
    )?);
    if original.configuration_hash.as_deref() != Some(&configuration_hash) {
        bail!("attribution withheld: harness permissions, arguments, or executable checks differ from original");
    }
    if o.user_mode != "verbatim" || o.sim_models.len() > 1 {
        bail!("attribution requires verbatim user turns to avoid mixing simulator effects with turn effects");
    }
    let n = user_turns(original).len();
    if o.turns.is_some_and(|t| t < n) {
        bail!("attribution must evaluate the whole task; omit --turns");
    }
    if turns.is_empty() {
        bail!("attribution requires at least one fork turn");
    }
    let mut unique = std::collections::BTreeSet::new();
    for &k in turns {
        if !unique.insert(k) {
            bail!("duplicate attribution turn {k}");
        }
    ...
    let harness = o.harness.unwrap_or(original.harness());
    let parent_id = o.run_id.clone().unwrap_or_else(|| {
        format!(
            "{}-attribute",
            make_run_id(original, harness, o.model.as_deref())
        )
    });
    let parent_dir = o
        .out_dir
    ...
    for turn in turns {
        let id = original
            .checkpoints
            .get(turn)
            .context("attribution withheld: missing checkpoint")?;
        let checkpoint = crate::checkpoint::load(id)?;
        crate::checkpoint::require_compatible(&checkpoint)?;
        if checkpoint.configuration_hash != configuration_hash {
            bail!("attribution withheld: checkpoint configuration differs");
        }
    }
    let _experiment_lock = if o.dry_run {
        None
    ...
    for &k in turns {
        ...
        let (n, passes) = match (&single, &matrix) {
            (_, Some(m)) => {
                execution_failed |= m.entries.iter().any(|e| {
                    e.errors > 0
                        || e.judge_score.is_none()
                        || e.outcome == "inconclusive"
                        || e.model != original.model
                });
                original_failed &= m
                    .entries
                    .iter()
                    .all(|e| e.original_judge_score.is_some_and(|s| s < o.pass_threshold));
            ...
            (Some(s), None) => {
                ...
                let entry = entry_from(
                    ...
                    false,
                    None,
                    user_turns(original).len(),
                    s,
                    o,
                );
                execution_failed |= entry.errors > 0
                    || entry.judge_score.is_none()
                    || entry.outcome == "inconclusive"
                    || entry.model != original.model;
                original_failed &= entry
                    .original_judge_score
                    .is_some_and(|s| s < o.pass_threshold);
        ...
        effects.push(TurnEffect {
            ...
            ci_low: lo,
            ci_high: hi,
            run_dir: opts.out_dir.clone().unwrap(),
        });
    }
    let point_of_commitment = if original_failed && !execution_failed && !o.dry_run {
        effects
            .iter()
            .filter(|e| e.ci_low > 0.0)
            .map(|e| e.turn)
            .max()
    } else {
        None
    ...
    if !original_failed {
        notes.push("Point of commitment withheld: the judge did not consistently score the original below the pass threshold (or judging failed).".into());
    }
    if execution_failed {
        notes.push("Point of commitment withheld: a harness or judge failed; inspect the saved run diagnostics.".into());
    }
    let att = Attribution {
        schema_version: 1,
    ...
    if !o.dry_run {
        ...
        fs::write(
            parent_dir.join("report.md"),
            render_attribution_markdown(&att),
        )?;
    }
    Ok(att)
}
...
pub fn render_attribution_text(a: &Attribution) -> String {
    if a.dry_run {
        return "Dry run: attribution planned; no outcomes evaluated or artifacts written.".into();
    }
    let c = colors();
    let mut out = vec![format!(
        "{}resample-at-turn attribution ({} replicates per turn){}",
        c.bold, a.replicates, c.reset
    )];
    out.push(format!(
        "{}{}{}{}",
        pad("turn", 6),
    ...
    for e in &a.effects {
        out.push(format!(
            "{}{}{}{}",
            pad(&e.turn.to_string(), 6),
            pad(&format!("{}/{} = {:.2}", e.passes, e.n, e.pass_rate), 12),
            pad(&format!("[{:.2}, {:.2}]", e.ci_low, e.ci_high), 16),
            e.run_dir.display()
        ));
    }
    out.push(String::new());
...
pub fn render_attribution_markdown(a: &Attribution) -> String {
    let mut md = vec![
        "# Resample-at-turn attribution".into(),
        String::new(),
        format!("- replicates per turn: {}", a.replicates),
        format!(
            "- point of commitment: {}",
            a.point_of_commitment
                .map(|t| format!("turn {t}"))
