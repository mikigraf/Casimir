...
case " $* " in
  *" --fake-empty "*) exit 0;;
  *" --fake-crash "*) echo '{"type":"system","subtype":"init","session_id":"crash"}'; echo 'deliberate failure' >&2; exit 42;;
  *" --fake-error "*) echo '{"type":"result","is_error":true,"result":"deliberate failure"}'; exit 0;;
esac
...
printf 'hi\n' > out.txt
case " $* " in
  *" --fake-commit "*) git add out.txt; git -c user.name=test -c user.email=test@example.com commit -qm 'agent commit';;
esac
