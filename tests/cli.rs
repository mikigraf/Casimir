...
use casimir::compare::{
    compare_sessions, end_state_similarity, first_divergent_turn, judge_sessions,
    judge_sessions_with, relativize, render_compare_markdown, render_compare_text,
    sequence_similarity, JudgeOpts,
};
use casimir::llm::LlmOpts;
use casimir::model::{
    actions, anti_patterns, classify_shell, files_touched, final_assistant_text,
    is_validation_command, lexicon_of, model_family, simulator_drift, stats, tool_one_liner,
    user_turns, ActionKind, Event, EventKind, Harness, Session, Usage,
};
...
fn fx(name: &str) -> PathBuf {
    if name.starts_with("fake-") {
        return fixture::executable(name);
    }
...
fn checkpointed_original(repo: &Path) -> Session {
    ...
    let native =
        claude_code::prepare_fork(&original, 2, &uuid::Uuid::new_v4().to_string(), repo).unwrap();
    ...
    let id = casimir::checkpoint::capture(casimir::checkpoint::Capture {
        cwd: repo,
        run_dir: &tmp("checkpoint-source"),
        native: Some(&native),
        harness: Harness::ClaudeCode,
        version: Some("casimir-fixture 1.0.0".into()),
        turn: 2,
        expected_conversation_turns: 1,
        prompt: &user_turns(&original)[1].text,
        configuration_hash: &configuration_hash,
        limit: casimir::checkpoint::DEFAULT_LIMIT,
    })
    .unwrap();
    original.checkpoints.insert(2, id);
    original.configuration_hash = Some(configuration_hash);
    original.evaluation = Some(
        json!({"outcome":"failed","execution":"completed","judgeModel":"fake:judge","passThreshold":7.0,"rubricHash":null,"judgeConfigurationHash":casimir::compare::judge_configuration_hash(&JudgeOpts::new(fake_llm("judge")),7.0)}),
    );
    original
}
...
fn simulator_cannot_claim_verbatim_or_goals_met_with_invalid_output() {
    ...
    for mode in ["sim-malformed", "sim-false-verbatim"] {
        ...
        let result =
            simulate_user_turn(&original, &original, 2, &fake_llm(mode), &mut state).unwrap();
        ...
        assert!(
            ...
            "rejected replies must not poison simulator memory"
        );
    }
}
...
fn skipped_turns_keep_original_alignment_in_pairs_and_exports() {
    setup_env();
    let mut original = claude_code::parse_file(&fx("claude-code.jsonl")).unwrap();
    original.events.push(Event::text(
        3,
        "2026-09-21T10:10:00Z",
        EventKind::User,
        "third original request",
    ));
    let repo = tmp_repo();
    ...
    let opts = RerunOpts {
        ...
        user_mode: "simulate".into(),
        sim_llm: fake_llm("sim-skip2"),
        judge_llm: fake_llm("brief"),
        quiet: true,
        ..Default::default()
    };
    ...
    let adapted = s
        .events
        .iter()
        .find(|e| e.kind == EventKind::User && e.turn == 2)
        .unwrap();
    ...
    let row: serde_json::Value =
        serde_json::from_str(std::fs::read_to_string(pairs.pairs_path).unwrap().trim()).unwrap();
    assert!(row["candidates"]
        .as_array()
        .unwrap()
        .iter()
        .any(|c| c["message"] == "third original request"));
...
fn simulator_transport_failures_save_reports_and_failed_execution() {
    ...
    for name in ["session.json", "record.jsonl", "report.json", "diff.patch"] {
...
fn fork_uses_native_log_from_saved_run_and_rejects_nonexistent_turn() {
    ...
    for (fixture, harness) in [
        ("claude-code.jsonl", Harness::ClaudeCode),
        ("codex.jsonl", Harness::Codex),
    ] {
        ...
        assert_eq!(
            user_turns(&casimir::adapters::parse_file(harness, &fork).unwrap()).len(),
            1
        );
        assert!(casimir::adapters::prepare_fork(
            harness,
            &original,
            3,
...
fn rerun_retains_commits_and_reports_failed_execution() {
    ...
    let committed = rerun(
        &original,
        &RerunOpts {
            out_dir: Some(tmp("committed")),
            extra_args: vec!["--fake-commit".into()],
            ..base.clone()
        },
        &mut no_log,
        &mut no_log,
    ...
    .unwrap();
    assert!(
        committed.diff.unwrap().patch.contains("+hi"),
        "committed edits remain part of the outcome"
    );
    assert!(Command::new("git")
        .args(["diff", "HEAD", "--exit-code"])
        .current_dir(&repo)
        .status()
        .unwrap()
        .success());
    for flag in ["--fake-error", "--fake-crash", "--fake-empty"] {
        ...
        assert!(
            matrix
                .entries
                .iter()
                .all(|e| !e.pass && e.completed_turns == 0 && e.errors > 0),
            "{flag}: failed prompts must not count as completed turns"
        );
        ...
        if flag == "--fake-crash" {
            assert!(std::fs::read_to_string(out_dir.join("r1/raw.jsonl"))
                .unwrap()
                .contains("init"));
        }
    }
...
fn matrix_freezes_rubric_and_isolates_workspaces() {
    ...
    let options = RerunOpts {
        ...
        replicates: 2,
        judge: true,
        judge_llm: fake_llm("judge"),
        ..Default::default()
    };
    let mut logs = Vec::new();
    let (_, matrix) = rerun_matrix(
        &original,
        &options,
        &mut |s| logs.push(s.to_string()),
        &mut no_log,
    )
    .unwrap();
    assert_eq!(
        logs.iter()
            .filter(|s| s.contains("drafting a per-session brief"))
            .count(),
        1
    );
    assert!(
        !repo.join("out.txt").exists(),
        "replicates must not mutate the source repository"
    );
    ...
    assert_eq!(
        std::fs::read(out_dir.join("r1/brief.json")).unwrap(),
        std::fs::read(out_dir.join("r2/brief.json")).unwrap()
    );
...
fn attribution_requires_outcomes_and_dry_runs_leave_no_artifacts() {
    ...
    let mut options = RerunOpts {
        workspace: repo.display().to_string(),
        out_dir: Some(output.clone()),
        quiet: true,
        replicates: 2,
        dry_run: true,
        judge_llm: fake_llm("judge-both-pass"),
        ..Default::default()
    };
    assert!(
        attribute(&original, &options, &[2], &mut no_log, &mut no_log)
            .unwrap_err()
            .to_string()
            .contains("requires --judge")
    );
    options.judge = true;
    assert!(
        attribute(&original, &options, &[2], &mut no_log, &mut no_log)
    ...
    assert!(
        attribute(&original, &options, &[2], &mut no_log, &mut no_log).is_err(),
        "missing original evidence cannot establish a rescued failure"
    );
    assert_eq!(
        wilson(0, 7).0,
        0.0,
        "floating point residue must not look like a positive effect"
    );
}
...
fn judge_schema_and_invalidity_are_enforced() {
    setup_env();
    let original = claude_code::parse_file(&fx("claude-code.jsonl")).unwrap();
    assert!(judge_sessions(
        &original,
        &original,
        None,
        None,
        &fake_llm("judge-malformed")
    )
    .is_err());
    let repo = tmp_repo();
    let options = RerunOpts {
        workspace: repo.display().to_string(),
        out_dir: Some(tmp("invalid-patch")),
        quiet: true,
        replicates: 2,
        judge: true,
        judge_llm: fake_llm("judge-invalid-patch"),
        ..Default::default()
    };
    let (_, matrix) = rerun_matrix(&original, &options, &mut no_log, &mut no_log).unwrap();
    assert!(
        matrix.unwrap().entries.iter().all(|e| !e.pass),
        "a high score cannot override an explicit invalidity finding"
    );
    let contradictory = judge_sessions(
        &original,
        &original,
        None,
        None,
        &fake_llm("judge-contradictory"),
    )
    .unwrap();
    assert!(contradictory
        .uncertainty
        .iter()
        .any(|u| u.contains("contradicts its scores")));
    assert_eq!(
        compare_sessions(&original, &original, None, None, Some(contradictory)).judge_assessment,
        "inconclusive"
    );
    let limited =
        judge_sessions(&original, &original, None, None, &fake_llm("judge-limited")).unwrap();
    assert!(!limited.limitations.is_empty());
    assert_eq!(
        compare_sessions(&original, &original, None, None, Some(limited)).judge_assessment,
        "passed"
    );
    let uncertain = judge_sessions(
        &original,
        &original,
        None,
        None,
        &fake_llm("judge-uncertain"),
    )
    .unwrap();
    assert_eq!(
        compare_sessions(&original, &original, None, None, Some(uncertain)).judge_assessment,
        "inconclusive"
    );
}
...
fn record_envelopes_are_ordered_unique_and_preserve_unanswered_calls() {
    ...
    assert!(rows
        .windows(2)
        .all(|r| r[0]["eventIndex"].as_u64() < r[1]["eventIndex"].as_u64()));
}
...
fn cli_accepts_standalone_patches_and_returns_failure_for_harness_errors() {
    ...
    let result = Command::new(env!("CARGO_BIN_EXE_casimir"))
        .args([
            "rerun",
            fx("claude-code.jsonl").to_str().unwrap(),
            "--workspace",
            repo.to_str().unwrap(),
            "--turns",
            "1",
    ...
    assert_eq!(
        std::fs::read(output.join("run/original.patch")).unwrap(),
        std::fs::read(patch).unwrap()
    );
    assert!(
        output.join("run/report.json").exists(),
        "a failed CLI run still saves its report"
    );
}
...
fn cli_rerun_uses_saved_reference_patch_after_workspace_changes() {
    ...
    assert_eq!(
        std::fs::read(output.join("original.patch")).unwrap(),
        expected
    );
}
...
fn judge_receives_tool_inputs_and_results_in_both_orders() {
    ...
    a.events.push(Event::tool_call(
        2,
        "",
        "verify",
        "Bash",
        json!({"command": "verification_evidence_marker"}),
    ));
    a.events.push(Event::tool_result(
        2,
        "",
        "verify",
        Some("Bash".into()),
        "verified content from actual tool result",
        false,
    ));
...
fn attribution_returns_nonzero_and_keeps_reports_for_harness_failures() {
    ...
    assert!(
        !output.join("attribution.json").exists(),
        "missing original evidence fails before model execution"
    );
}
...
fn simulator_noops_and_goals_met_have_explicit_completion_semantics() {
    ...
    let original = claude_code::parse_file(&fx("claude-code.jsonl")).unwrap();
    let repo = tmp_repo();
    for (mode, judged, expected_pass) in [
        ("sim-noop", true, true),
        ("sim-goals-met", true, false),
        ("sim-goals-met", false, false),
        ("sim-stop", true, false),
    ] {
        let opts = RerunOpts {
            workspace: repo.display().to_string(),
            out_dir: Some(tmp("sim-completion")),
            quiet: true,
            replicates: 2,
            user_mode: "simulate".into(),
            sim_llm: fake_llm(mode),
            judge: judged,
            judge_llm: fake_llm("judge"),
            ..Default::default()
        };
...
fn diff_capture_from_subdirectory_keeps_staged_and_untracked_paths() {
    ...
    assert!(
        !empty.informative,
        "empty reference patches cannot support an agreement score"
    );
}
...
fn mismatched_or_failed_controls_cannot_establish_a_noise_floor() {
    ...
    let options = RerunOpts {
        workspace: repo.display().to_string(),
        out_dir: Some(tmp("model-mismatch")),
        quiet: true,
        control: true,
        ..Default::default()
    };
    ...
    original.model = Some("fake-model".into());
    let options = RerunOpts {
        out_dir: Some(tmp("failed-control")),
        extra_args: vec!["--fake-error".into()],
        ..options
    };
    let (_, matrix) = rerun_matrix(&original, &options, &mut no_log, &mut no_log).unwrap();
    assert!(matrix
        ...
        .groups
        .iter()
        .all(|g| g.exceeds_control.is_none()));
}
...
fn claude_code_parses_log_into_normalized_session() {
    ...
    assert_eq!(
        turns[0].text, "Add a greet(name) function to lib.py and a test for it",
    ...
    let err = s
        .events
        .iter()
        .find(|e| e.kind == EventKind::ToolResult && e.result.as_ref().unwrap().is_error)
        .unwrap();
...
fn codex_parses_rollout_skips_injected_context_maps_tools_and_usage() {
    assert!(codex::is_injected(
        "<recommended_plugins>host recommendations</recommended_plugins>"
    ));
    ...
    assert_eq!(
        turns[0].text,
        "Add a greet(name) function to lib.py and a test for it"
    );
    let calls: Vec<&Event> = s
        .events
        .iter()
    ...
    let names: Vec<&str> = calls
        .iter()
        .map(|e| e.tool.as_ref().unwrap().name.as_str())
        .collect();
    assert_eq!(names, ["shell", "apply_patch", "shell", "apply_patch"]);
    assert_eq!(
        calls[0].tool.as_ref().unwrap().input["command"],
        json!(["bash", "-lc", "cat lib.py"])
    );
    let results: Vec<&Event> = s
        .events
        .iter()
        .filter(|e| e.kind == EventKind::ToolResult)
        .collect();
    assert_eq!(
        results[0].result.as_ref().unwrap().output,
        "def add(a, b):\n    return a + b\n",
        "JSON-wrapped output unwrapped"
    );
    assert!(!results[0].result.as_ref().unwrap().is_error);
    assert!(
        results[2].result.as_ref().unwrap().is_error,
        "Exit code: 127 detected"
    );
    assert_eq!(
        results[2].result.as_ref().unwrap().name.as_deref(),
        Some("shell")
    );
    let st = stats(&s);
    ...
    let files: Vec<(String, Vec<String>)> = files_touched(&s)
        .into_iter()
        .map(|f| (f.path, f.ops))
        .collect();
    assert_eq!(
        files,
        [
            ("lib.py".to_string(), vec!["update".to_string()]),
            ("test_lib.py".to_string(), vec!["add".to_string()])
        ]
    );
    assert_eq!(
        final_assistant_text(&s, None),
        "Done: greet now defaults to \"World\"."
    );
...
fn format_detection_and_session_resolution() {
    setup_env();
    assert_eq!(
        load_session_file(&fx("claude-code.jsonl")).unwrap().harness,
        Some(Harness::ClaudeCode)
    );
    assert_eq!(
        load_session_file(&fx("codex.jsonl")).unwrap().harness,
        Some(Harness::Codex)
    );
    assert_eq!(
        resolve_session(fx("codex.jsonl").to_str().unwrap())
            .unwrap()
            .id,
    ...
    assert_eq!(
        load_session_file(&exported).unwrap().events.len(),
        orig.events.len()
    );
    let err = resolve_session("definitely-not-a-session-xyz")
        .unwrap_err()
        .to_string();
...
fn tool_one_liners_and_file_inference_from_shell() {
    let ev = Event::tool_call(
        1,
        "2026-01-01T00:00:00Z",
        "x",
        "Bash",
        json!({ "command": "cat > a/b.txt <<'EOF'\nhi\nEOF\n; tee out.log" }),
    );
    assert_eq!(
        tool_one_liner(&ev, 40),
        "Bash: cat > a/b.txt <<'EOF' hi EOF ; te…"
    );
    let s = Session {
        events: vec![ev],
        ..Default::default()
...
fn renderers_produce_transcript_and_markdown() {
    ...
    assert!(
        !text.contains("Explore the repo"),
        "sidechains hidden by default"
    );
    ...
    let md = render_markdown(
        &codex::parse_file(&fx("codex.jsonl")).unwrap(),
        &RenderOpts::default(),
    );
...
fn compare_relativizes_paths_across_harnesses() {
    ...
    assert_eq!(
        relativize("/work/demo/x/y.py", Some("/work/demo")),
        "x/y.py"
    );
    assert_eq!(
        relativize("/elsewhere/y.py", Some("/work/demo")),
        "/elsewhere/y.py"
    );
...
fn extract_json_tolerates_prose_and_fences() {
    ...
    assert_eq!(
        extract_json("prefix {\"action\":\"stop\",\"message\":\"\"} suffix"),
        Some(json!({ "action": "stop", "message": "" }))
    );
...
fn claude_code_stream_json_mapping() {
    ...
    assert_eq!(
        claude_code::record_to_events(&synthetic, 1)[0].kind,
        EventKind::Error
    );
}
...
fn codex_exec_json_event_mapping() {
    ...
    assert!(codex::exec_event_to_events(
    ...
    let evs = codex::exec_event_to_events(
        &json!({ "type": "item.completed", "item": { "id": "i", "type": "command_execution", "command": "ls", "aggregated_output": "a\n", "exit_code": 2 } }),
        1,
        &mut state,
    );
    assert_eq!(evs[0].kind, EventKind::ToolCall);
    assert!(evs[1].result.as_ref().unwrap().is_error);
    codex::exec_event_to_events(
        &json!({ "type": "turn.completed", "usage": { "input_tokens": 3, "cached_input_tokens": 1, "output_tokens": 2 } }),
        1,
        &mut state,
    );
    assert_eq!(
...
fn rerun_drives_fake_claude_harness_end_to_end() {
    ...
    assert!(
        diff.files.iter().any(|f| f.path == "out.txt"),
...
fn rerun_creates_worktree_at_base_commit() {
    ...
    assert!(
        res.workspace.dir.join("out.txt").exists(),
        "fake harness wrote into the worktree"
    );
    assert!(
        !repo.join("out.txt").exists(),
...
fn copilot_parses_session_dir_and_lists() {
    ...
    assert_eq!(
        s.title.as_deref(),
        Some("Add greet function to lib.py"),
        "block-scalar summary parsed"
    );
    ...
    assert_eq!(
        names,
        ["bash", "edit", "create"],
        "toolRequest and execution_start for the same call are not duplicated"
    );
    let st = stats(&s);
    ...
    assert_eq!(
        ...
        "shutdown modelMetrics, uncached input derived"
    );
    let files: Vec<String> = files_touched(&s).into_iter().map(|f| f.path).collect();
    assert_eq!(files, ["/work/demo/lib.py", "/work/demo/test_lib.py"]);
    assert_eq!(
        load_session_file(&dir).unwrap().harness,
        Some(Harness::Copilot)
    );
    assert_eq!(
        load_session_file(&dir.join("events.jsonl"))
            .unwrap()
            .events
            .len(),
        s.events.len()
...
fn gemini_parses_jsonl_with_upserts_injected_context_and_project_root() {
    ...
    assert_eq!(
        files_touched(&s)
            ...
            .map(|f| f.path)
            .collect::<Vec<_>>(),
        ["/work/demo/test_lib.py"]
    );
    assert_eq!(
        s.events
            .iter()
            .find(|e| e.kind == EventKind::Thinking)
            .unwrap()
            .text_str(),
        "**Inspecting lib.py** I should read the file first."
    );
    assert_eq!(
        load_session_file(&file).unwrap().harness,
        Some(Harness::Gemini)
    );
...
fn judge_runs_both_orders_and_flags_order_sensitivity() {
    ...
    assert!(
        j.order_sensitive,
        "a judge that always prefers the first candidate must be caught"
    );
    ...
    assert_eq!(
        j.passes[1].winner, "B",
        "second pass winner mapped back to real labels"
    );
    assert!(
        (j.score_a - 7.0).abs() < 1e-9 && (j.score_b - 7.0).abs() < 1e-9,
        "scores averaged across orders"
    );
    assert!(j.close);
    assert_eq!(j.model, "fake:judge-flip");
    // a consistent judge keeps its verdict
    let mut b2 = b.clone();
    b2.events.push(Event::text(
        2,
...
fn simulator_retries_ungrounded_replies_and_reports_stop_reasons() {
    ...
    rerun_s.events.push(Event::text(
        1,
    ...
    assert_eq!(
        state.memory.len(),
        1,
        "only the accepted reply's note is kept"
    );
    let stop =
        simulate_user_turn(&original, &rerun_s, 2, &fake_llm("sim-stop"), &mut state).unwrap();
    assert!(stop.message.is_none());
    assert_eq!(stop.stop_reason.as_deref(), Some("out_of_scope"));
    let vb = simulate_user_turn(
        &original,
        &rerun_s,
        2,
        &fake_llm("sim-verbatim"),
        &mut state,
    )
    .unwrap();
    assert!(vb.verbatim);
    assert_eq!(
        vb.message.as_deref(),
        Some("Also make greet default to 'World' when no name is given")
    );
}
...
fn end_state_and_tool_sequence_similarity() {
    ...
    assert!(
        (es.content_similarity - 0.5).abs() < 1e-9,
        "lib.py identical (1.0), test_lib.py missing (0.0)"
    );
    ...
    assert!(
        (sequence_similarity(&v(&["Bash", "Edit", "Write"]), &v(&["Bash", "Write"])) - 0.8).abs()
            < 1e-9
    );
...
fn simulated_rerun_records_simulator_and_marks_turns() {
    ...
    let opts = RerunOpts {
        ...
        quiet: true,
        user_mode: "simulate".into(),
        sim_llm: fake_llm("sim-verbatim"),
        judge: true,
        judge_llm: fake_llm("judge-consistent"),
        run_id: Some("t-sim".into()),
        original_diff: Some(original_diff),
        ..Default::default()
    };
    let res = rerun(&original, &opts, &mut no_log, &mut no_log).unwrap();
    let session = res.session.unwrap();
    assert_eq!(
        session.simulator.as_ref().map(|s| s.model.as_str()),
        Some("fake:sim-verbatim")
    );
    assert_eq!(
        session.simulator.as_ref().map(|s| s.backend.as_str()),
        Some("cmd")
    );
    let second = session
        .events
    ...
    let sim = second
        .simulated
        .as_ref()
        .expect("second turn marked as simulated");
    assert!(sim.verbatim);
    ...
    let report = res.report.unwrap();
    assert_eq!(
        report.b.simulator_model.as_deref(),
        Some("fake:sim-verbatim")
    );
    ...
    assert_eq!(
        j.winner, "B",
        "fake harness output mentions 'Working on', which the consistent fake judge prefers"
    );
    let es = report
        .end_state
        .expect("end state computed from the supplied original diff and the captured rerun diff");
    assert!(
        (es.score - 1.0).abs() < 1e-9,
        "fake harness wrote the same out.txt as the supplied original diff"
    );
...
fn replicated_rerun_aggregates_pass_rates() {
    ...
    assert!(
        m.entries.iter().all(|e| !e.pass && e.errors == 0),
        "clean execution without evaluation remains inconclusive"
    );
    ...
    let opts2 = RerunOpts {
        ...
        user_mode: "simulate".into(),
        sim_llm: fake_llm("sim-verbatim"),
        sim_models: vec!["fake:sim-verbatim".into(), "fake:sim-stop".into()],
        run_id: Some("t-matrix2".into()),
        ..Default::default()
    };
    ...
    let stop_group = m2
        .groups
        .iter()
        .find(|g| g.simulator_model.as_deref() == Some("fake:sim-stop"))
        .unwrap();
    assert!(
        !stop_group.pass_pow_k,
        "an out_of_scope stop before the last turn is not a pass"
    );
    let ok_group = m2
        .groups
        .iter()
        .find(|g| g.simulator_model.as_deref() == Some("fake:sim-verbatim"))
        .unwrap();
...
fn action_taxonomy_maps_tools_and_shell_commands() {
    ...
    assert_eq!(
        classify_shell("cat src/lib.py"),
        (ActionKind::FileRead, false, Some("src/lib.py".into()))
    );
    assert_eq!(
        classify_shell("cargo test -q"),
        (ActionKind::Command, true, None)
    );
    assert_eq!(classify_shell("cd src && ls").0, ActionKind::Navigate);
    assert_eq!(
        classify_shell("cat > out.txt <<'EOF'\nhi\nEOF").0,
    ...
    assert!(
        actions(&s)[3].validation,
        "npm test is a validation command even through a generic shell tool"
    );
    let st = stats(&s);
...
fn anti_patterns_follow_the_published_rules() {
    ...
    let mut events: Vec<Event> = (0..10)
        .map(|i| call(1, &format!("r{i}"), "Grep", json!({ "pattern": "x" })))
        .collect();
    events.push(call(1, "w", "Write", json!({ "file_path": "/w/a.py" })));
    let ap = anti_patterns(&Session {
        events: events.clone(),
        ..Default::default()
    });
    ...
    assert!(
        anti_patterns(&no_churn).reread_churn_files.is_empty(),
        "a write in between resets the window"
    );
    // failed-action share from tool results
    let mut failing = Session {
        events: vec![
            call(1, "x", "Bash", json!({ "command": "ls" })),
            call(1, "y", "Bash", json!({ "command": "ls" })),
        ],
        ..Default::default()
    };
    failing.events.push(Event::tool_result(
        1,
        "2026-01-01T00:00:01Z",
        "x",
        Some("Bash".into()),
    ...
    assert!(
        ap3.verification_skip,
        "the last write (turn 2 Edit) is not followed by a test run"
    );
}
...
fn divergence_recall_verdict_and_wilson() {
    ...
    let idx = b
        .events
        .iter()
        .position(|e| e.turn == 2 && e.kind == EventKind::ToolCall)
        .unwrap();
    ...
    assert_eq!(
        first_divergent_turn(&a, &c),
        Some(2),
        "missing turn counts as divergence"
    );
    ...
    assert!(
        es.content_similarity < es.recall,
        "symmetric Jaccard penalizes the extra edits, recall does not"
    );
    ...
    for harness in Harness::all() {
        for mode in ["empty", "truncated", "failed", "bare-result"] {
            let opts = casimir::adapters::RunOpts {
                prompt: "hello".into(),
                bin: Some(fx("fake-stream.py").display().to_string()),
                extra_args: vec!["--fake-mode".into(), mode.into()],
                ..Default::default()
            };
            let result = casimir::adapters::run_turn(harness, &opts, &mut |_| {});
            assert!(
                result.as_ref().map_or(true, |r| r.is_error),
                "{harness}/{mode} must fail"
            );
            if let Ok(res) = result {
                assert!(
                    res.events.iter().any(|e| e.kind == EventKind::Error),
                    "failure must be visible in the transcript"
                );
                if mode != "empty" {
                    assert!(!res.raw.is_empty(), "partial raw output must be saved");
                }
            }
        }
    }
...
fn copilot_and_gemini_reruns_resume_and_save_artifacts() {
    ...
    for harness in [Harness::Copilot, Harness::Gemini] {
        ...
        let opts = RerunOpts {
            ...
            workspace: tmp_repo().display().to_string(),
        ...
        assert!(
            out.join("session.json").exists()
                && out.join("report.md").exists()
                && out.join("raw.jsonl").exists()
        );
        assert!(std::fs::read_to_string(out.join("diff.patch"))
            .unwrap()
            .contains("out.txt"));
        if harness == Harness::Gemini {
            assert_eq!(stats(&session).usage.output, 60);
        }
    }
}
...
fn gemini_result_error_without_error_event_is_visible() {
    let opts = casimir::adapters::RunOpts {
        cwd: Some(tmp("gemini-error")),
        prompt: "hello".into(),
        bin: Some(fx("fake-stream.py").display().to_string()),
        extra_args: vec!["--fake-mode".into(), "result-error".into()],
        ..Default::default()
    };
...
fn control_group_measures_the_noise_floor() {
    ...
    assert!(
        target.mean_distance > 0.0,
        "fake harness trajectory differs from the original"
    );
    assert!(m.entries.iter().all(|e| e.first_divergent_turn == Some(1)));
    assert!(
        out_dir.join("control-r1/record.jsonl").exists(),
        "record envelopes written"
    );
...
fn fork_at_turn_prepares_a_truncated_transcript_and_resumes() {
    ...
    let opts = RerunOpts {
        ...
        quiet: true,
        from_turn: Some(2),
        intervention: Some("Make greet default to 'Earth' instead".into()),
        run_id: Some("t-fork".into()),
        ..Default::default()
    };
    let res = rerun(&original, &opts, &mut no_log, &mut no_log).unwrap();
    let session = res.session.unwrap();
    // prefix preserved verbatim from the original
    let prefix_tools: Vec<&str> = session
        .events
        .iter()
        .filter(|e| e.turn == 1 && e.kind == EventKind::ToolCall && !e.sidechain)
        .map(|e| e.tool.as_ref().unwrap().name.as_str())
        .collect();
    ...
    assert!(session
        .events
        .iter()
        .any(|e| e.kind == EventKind::System && e.subtype.as_deref() == Some("fork")));
    let t2 = session
        .events
        .iter()
        .find(|e| e.kind == EventKind::User && e.turn == 2)
    ...
    let reply = session
        .events
        .iter()
        .find(|e| e.kind == EventKind::Assistant && e.turn == 2)
        .unwrap();
    assert!(reply
        .text_str()
        .contains("Working on: Make greet default to 'Earth'"));
    ...
    assert_eq!(
        forked.cwd.as_deref(),
        Some(res.workspace.dir.display().to_string().as_str())
    );
    ...
    let path = codex::prepare_fork(
        &codex_orig,
        2,
        "eeeeeeee-1111-4222-8333-666666666666",
        &repo,
    )
    .unwrap();
...
fn attribution_finds_the_point_of_commitment() {
    ...
    assert_eq!(
        att.effects[0].passes, 2,
        "fake harness always completes cleanly"
    );
...
fn judge_repeats_measure_position_bias_and_test_retest() {
    ...
    let jo = JudgeOpts {
        repeats: 2,
        ..JudgeOpts::new(fake_llm("judge-flip"))
    };
    let j = judge_sessions_with(&a, &b, None, None, &jo).unwrap();
    assert_eq!(j.repeats, 2);
    assert_eq!(j.passes.len(), 4);
    assert!(
        (j.first_slot_win_rate - 1.0).abs() < 1e-9,
        "the biased fake judge always picks the first slot"
    );
    ...
    let jo2 = JudgeOpts {
        llm: fake_llm("judge-consistent"),
        repeats: 1,
        brief: None,
        model_a: Some("claude-opus-5".into()),
        model_b: Some("gpt-5-codex".into()),
    };
    ...
    assert!(j2
        ...
        .as_deref()
        .unwrap()
        .contains("candidate A only"));
    let same_both = JudgeOpts {
        model_a: Some("claude-opus-5".into()),
        model_b: Some("claude-sonnet-5".into()),
        ..anthropic_judge.clone()
    };
    assert!(
        judge_sessions_with(&a, &b, None, None, &same_both)
            .unwrap()
            .family_warning
            .is_none(),
        "symmetric families raise no warning"
    );
    let text = render_compare_text(
        &compare_sessions(&a, &b, None, None, Some(j.clone())),
        "A",
        "B",
    );
...
fn brief_rubric_invalid_reasons_and_intent_coverage() {
    ...
    let jo = JudgeOpts {
        brief: Some(loaded.clone()),
        ..JudgeOpts::new(fake_llm("judge-consistent"))
    };
    let j = judge_sessions_with(&a, &b, None, None, &jo).unwrap();
    assert!(j.rubric);
    ...
    rerun_s
        .events
        .iter_mut()
        .filter(|e| e.kind == EventKind::User && e.turn == 2)
        .for_each(|e| {
            e.text = Some("please use World as the default".into());
            e.simulated = Some(casimir::model::Simulated {
                verbatim: false,
                reason: "adapted".into(),
                grounded_in: vec![2],
                action: Some("redirect".into()),
            });
        });
    let ic = intent_coverage(&loaded, &rerun_s, &fake_llm("intent"))
        .unwrap()
        .unwrap();
    ...
    assert!(
        intent_coverage(&loaded, &a, &fake_llm("intent"))
            .unwrap()
            .is_none(),
        "no simulated turns → nothing to cover"
    );
}
...
fn lexicon_counters_and_simulator_drift() {
    let lx = lexicon_of([
        "ok",
        "Please fix lib.py, thanks",
        "Maybe use greet_name instead?",
        "Fix — now",
    ]);
    assert_eq!(lx.turns, 4);
    assert!(
        (lx.short_turn_rate - 0.5).abs() < 1e-9,
        "'ok' and 'Fix — now' are <= 3 words"
    );
    ...
    assert!(
        (lx.identifier_tokens_per_turn - 0.5).abs() < 1e-9,
        "lib.py and greet_name"
    );
    let original = claude_code::parse_file(&fx("claude-code.jsonl")).unwrap();
    ...
    let text = render_compare_text(
        &compare_sessions(&original, &rerun_s, None, None, None),
        "A",
        "B",
    );
...
fn simulated_rerun_with_brief_noop_and_pairs_export() {
    ...
    let opts = RerunOpts {
        ...
        quiet: true,
        user_mode: "simulate".into(),
        sim_llm: fake_llm("sim-noop"),
        judge: true,
        judge_llm: fake_llm("brief"),
        run_id: Some("t-noop".into()),
        ..Default::default()
    };
    let res = rerun(&original, &opts, &mut no_log, &mut no_log).unwrap();
    let session = res.session.unwrap();
    assert!(session
        .events
        .iter()
        .any(|e| e.kind == EventKind::System && e.subtype.as_deref() == Some("simulator-noop")));
    assert_eq!(user_turns(&session).len(), 1);
    assert!(
        out_dir.join("brief.json").exists(),
        "brief drafted into the run directory"
    );
    let brief = Brief::load(&out_dir.join("brief.json")).unwrap();
    assert_eq!(brief.intents.len(), 3);
    let report = res.report.unwrap();
    assert!(
        report.judge.as_ref().unwrap().rubric,
        "judge used the drafted rubric"
    );
    // an adapted turn: action label recorded, drift + intent coverage in the report, pairs exportable
    let out2 = tmp("adapt");
    let opts2 = RerunOpts {
        workspace: repo.display().to_string(),
        out_dir: Some(out2.clone()),
        quiet: true,
        user_mode: "simulate".into(),
        sim_llm: fake_llm("sim-adapt"),
        judge: true,
        judge_llm: fake_llm("brief"),
        brief: Some(out_dir.join("brief.json")),
        run_id: Some("t-adapt".into()),
        ..Default::default()
    };
    ...
    let t2 = s2
        ...
        .unwrap();
    assert_eq!(
        t2.simulated.as_ref().unwrap().action.as_deref(),
        Some("redirect")
    );
    ...
    let ic = r2
        .intent_coverage
        .expect("intent coverage computed when judged with a brief");
    assert!((ic.recall - 1.0).abs() < 1e-9);
    let pairs_dir = tmp("pairs");
    let exp = export_pairs(&[out2.clone(), out_dir.clone()], &pairs_dir).unwrap();
    assert_eq!(
        exp.n, 1,
        "only the adapted turn yields a pair; the no-op run has none"
    );
    let key: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&exp.key_path).unwrap()).unwrap();
    let (pair_id, real) = key
        .as_object()
        .unwrap()
        .iter()
        .next()
        .map(|(k, v)| (k.clone(), v["real"].as_str().unwrap().to_string()))
        .unwrap();
    let line = std::fs::read_to_string(&exp.pairs_path).unwrap();
    assert!(
        line.contains("please use World as the default")
            && line.contains("Also make greet default to 'World'")
    );
    ...
    std::fs::write(
        pairs_dir.join("answers.json"),
        format!("{{\"{pair_id}\": \"{wrong}\"}}"),
    )
    .unwrap();
    ...
    std::fs::write(
        pairs_dir.join("answers2.json"),
        format!("{{\"{pair_id}\": \"{real}\"}}"),
    )
    .unwrap();
    assert_eq!(
        score_pairs(&exp.key_path, &pairs_dir.join("answers2.json"))
            .unwrap()
            .simulator_passed,
        0
    );
...
fn matrix_reports_judge_quality_spread_and_notes() {
    ...
    let opts = RerunOpts {
        ...
        replicates: 2,
        judge: true,
        judge_llm: fake_llm("judge-flip"),
        user_mode: "simulate".into(),
        sim_llm: fake_llm("sim-verbatim"),
        sim_models: vec!["fake:sim-verbatim".into(), "fake:sim-adapt".into()],
        control: true,
        run_id: Some("t-mx".into()),
        ..Default::default()
    };
    let (_, m) = rerun_matrix(&original, &opts, &mut no_log, &mut no_log).unwrap();
    let m = m.unwrap();
    assert_eq!(
        m.entries.len(),
        8,
        "2 targets (control + target) × 2 simulators × 2 replicates"
    );
    ...
    for g in &m.groups {
        ...
        assert!(g
            ...
            .as_deref()
            .unwrap()
            .contains("flipped on order swap"));
    ...
    for g in m.groups.iter().filter(|g| !g.control) {
        assert!(
            g.exceeds_control.is_some(),
            "each target has a control sharing its simulator"
        );
    }
    assert_eq!(
        m.simulator_spread.len(),
        2,
        "one spread per target (control and target)"
    );
